...
const getItemYear = (
  item: Pick<AwardResult, "release_date" | "first_air_date">,
) => {
...
export async function GET(request: Request) {
  try {
    ...
    const awardLimitParam = Number(
      requestUrl.searchParams.get("award_limit") ?? 12,
    );
    const page = Number.isFinite(pageParam) && pageParam > 0 ? pageParam : 1;
    const limit =
      Number.isFinite(limitParam) && limitParam > 0 ? limitParam : 18;
    const awardLimit =
      Number.isFinite(awardLimitParam) && awardLimitParam > 0
        ? awardLimitParam
        : 12;
    ...
    const maxTotalPages = Math.max(
      1,
      ...fulfilledResponses.map((entry) =>
        Number.isFinite(entry.total_pages) ? entry.total_pages : 1,
      ),
    );
    ...
    const normalizedItems = [...byId.values()].sort(
      (a, b) => getItemScore(b) - getItemScore(a),
    );

    const results = normalizedItems.slice(0, limit);
    const awardSections = buildAwardSections(
      normalizedItems,
      awardSearches.map((award) => award.label),
      awardLimit,
    );
    ...
    if (results.length === 0) {
      ...
      const sortedFallback = [...uniqueFallback.values()].sort(
        (a, b) => getItemScore(b) - getItemScore(a),
      );
      ...
      const fallbackTotalPages = Math.max(
        1,
        Math.ceil(sortedFallback.length / limit),
      );
      const awardSectionsFallback = buildAwardSections(
        sortedFallback,
        awardSearches.map((award) => award.label),
        awardLimit,
      );
